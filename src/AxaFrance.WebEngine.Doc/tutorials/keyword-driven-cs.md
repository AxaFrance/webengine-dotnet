...
Along with the modelling, it is necessary to define also test data to be used for each action keyword. that is Test data to manipulate application under test and test data to verify expected result.
...
All action keyword must inherent from <xref:AxaFrance.WebEngine.SharedActionBase>. For Web based keyword, inherent from <xref:AxaFrance.WebEngine.Web.SharedActionWeb>, then implement `DoAction` and `DoCheckpoint`
