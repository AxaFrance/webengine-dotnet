...
All action keyword must inherent from <xref:AXA.WebEngine.SharedActionBase>. For Web based keyword, inherent from <xref:AXA.WebEngine.Web.SharedActionWeb>, then implement `DoAction` and `DoCheckpoint`
